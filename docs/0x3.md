...
scanf("%s", name);
```
Burda yine `scanf()`, doğrudan sadece `%s` formatı ile kullanıldığından overflow mümkün.
İsmimizi sorduktan sonra bu sefer ilginç birşey yapıyor, ismizi doğrulamamızı istiyor:
```c
printf(name);
printf("? [yes/no]\n");
```
Bu sorunun cevabı yine bir `scanf()` çağrısı ile `answer` bufferına yazılıyor. Yine
...
kullanmıyor. Bunun yerine `-fstack-protector` kullanılmış. Bu da demek ki "stack protector"
