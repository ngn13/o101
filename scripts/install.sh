...
rm -f "${MNTDIR}/root"/0x*/*.elf "${MNTDIR}/root"/0x*/*.py \
  "${MNTDIR}/root"/*/0x*/*.elf "${MNTDIR}/root"/*/0x*/*.py

info "Installing additional packages inside chroot"
chroot "${MNTDIR}" /bin/bash -c "apt install -y vim cowsay dhcpcd openssh-server gdb tmux wget strace gawk bison"
chroot "${MNTDIR}" /bin/bash -c "apt install -y python3-pwntools python3-capstone python3-pkg-resources"
...
info "Copying glibc source"
cp -rv "${DISTDIR}/${glibc_dir}" "${MNTDIR}/root/${glibc_dir}"
...
info "Copying noaslr script & service"
